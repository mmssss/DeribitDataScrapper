...
async def scrap_available_instruments(currency: Currency, cfg):
    ...
    make_subscriptions_list = await send_request(
    ...
    available_maturities = pd.DataFrame(
        np.unique(list(map(lambda x: x["instrument_name"].split("-")[1], answer)))
    )
    available_maturities.columns = ["DeribitNaming"]
    available_maturities["RealNaming"] = pd.to_datetime(
        available_maturities["DeribitNaming"], format="%d%b%y"
    )
    available_maturities = available_maturities.sort_values(
        by="RealNaming"
    ...
    selected = list(
        map(
            lambda x: x["instrument_name"],
            list(
                filter(
                    lambda x: (selected_maturity in x["instrument_name"])
                    and (x["option_type"] == "call" or "put"),
                    answer,
                )
            ),
        )
    ...
    get_underlying = await send_request(
        get_ticker_by_instrument_request(selected[0]),
        show_answer=False,
        URL_TO_SCRAP=cfg,
    )
...
def subscription_map(scrapper, conf: dict) -> dict[str, AbstractSubscription]:
    ...
    for sub in conf["orderBookScrapper"]["scrapper_body"]:
        if sub == "OrderBook":
            res_dict[
                "OrderBook"
            ]: OrderBookSubscriptionCONSTANT = OrderBookSubscriptionCONSTANT(
                scrapper=scrapper, order_book_depth=conf["orderBookScrapper"]["depth"]
            )
        elif sub == "Trades":
            res_dict["Trades"]: TradesSubscription = TradesSubscription(
                scrapper=scrapper
            )
        elif sub == "Portfolio":
            res_dict[
                "Portfolio"
            ]: UserPortfolioSubscription = UserPortfolioSubscription(scrapper=scrapper)
        elif sub == "OwnOrderChange":
            res_dict["OwnOrderChange"]: OwnOrdersSubscription = OwnOrdersSubscription(
                scrapper=scrapper
            )
    return res_dict


def net_databases_to_subscriptions(
    scrapper: DeribitClient,
) -> dict[AbstractSubscription, AbstractDataManager]:
    """
...
class DeribitClient(Thread, WebSocketApp):
    ...
    def __init__(
        ...
        enable_database_record = bool(
            self.configuration["orderBookScrapper"]["enable_database_record"]
        )

        self.only_API_orders = self.configuration["orderBookScrapper"][
            "only_api_orders_processing"
        ]
        # Extra like BTC-PERPETUAL
        for _instrument_name in self.configuration["orderBookScrapper"][
        ...
        self.instrument_name_instrument_id_map: AutoIncrementDict[
            str, AbstractInstrumentInfo
        ] = AutoIncrementDict(
            path_to_file=self.configuration["record_system"][
        ...
        self.subscriptions_objects = subscription_map(
            scrapper=self, conf=self.configuration
        )
    ...
    def _on_message(self, websocket, message):
            ...
            if "order" in response["result"]:
                if "OwnOrderChange" in self.subscriptions_objects:
                    asyncio.run_coroutine_threadsafe(
                        self.subscriptions_objects[
                            "OwnOrderChange"
                        ].process_response_from_server(response=response),
                        loop=self.loop,
                    )
        # subscriptions
        if "method" in response:
            ...
            for action, sub in self.subscriptions_objects.items():
                asyncio.run_coroutine_threadsafe(
                    sub.process_response_from_server(response=response), loop=self.loop
                )
                    ...
                    if "token_type" not in response["result"]:
                        if response["result"] != "ok":
                            for dict_obj in response["result"]:
                                asyncio.run_coroutine_threadsafe(
                                    self.instrument_manager.process_validation(
                                        dict_obj
                                    ),
                                    self.loop,
                                )
    ...
    def _process_error_callbacks(self, response: dict):
        if "message" in response["error"]:
            if "not_enough_funds" in response["error"]["message"]:
                ...
                asyncio.run_coroutine_threadsafe(
                    self.order_manager.not_enough_funds(callback=response),
                    loop=self.loop,
                )
            elif "price_too_high" in response["error"]["message"]:
                logging.warning(f"{response}")
                asyncio.run_coroutine_threadsafe(
                    self.order_manager.price_too_high(callback=response), loop=self.loop
                )
            else:
    ...
    def send_block_sync_request(
        ...
        if _private == "private":
            client_id = (
                ...
                if self.configuration["orderBookScrapper"]["test_net"]
                else self.configuration["user_data"]["production"]["client_id"]
            )

            client_secret = (
                self.configuration["user_data"]["test_net"]["client_secret"]
                if self.configuration["orderBookScrapper"]["test_net"]
                else self.configuration["user_data"]["production"]["client_secret"]
            )
            ...
            auth = session.get(
                f"https://{_hist}deribit.com/api/v2/public/auth", params=auth_params
            )
